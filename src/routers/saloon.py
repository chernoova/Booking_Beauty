...
async def current_time_table(callback_query: CallbackQuery, state: FSMContext):
...
async def add_time_table_price(message: Message, state: FSMContext, session: AsyncSession):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=f'{i.name} - {price} ✅', callback_data='nothing')]
                         ...
                         services])
    #for i in services:
    #    if i.id in datas['services'].keys():
    #        kb.add(InlineKeyboardButton(text=f'{i.name} - {price} ✅', callback_data='nothing'))
    #    else:
    #        kb.add(InlineKeyboardButton(text=i.name,
    #                                    callback_data=CallbackFabricService(id=i.id, name=i.name).pack()))
    #kb = kb.as_markup()
    kb.inline_keyboard.append([InlineKeyboardButton(text='Больше не нужно', callback_data='end_service')])
    await state.set_state(AddTimeTableState.services)
