...
class UserQueries:
    ...
    async def get_all_datetimes(session: AsyncSession, service_id: int, beauty_saloon_id: int, specialist_id: int,
        ...
        try:
            return sorted(list((await session.scalars(
                ...
                .join(BeautySaloon).join(Specialist).select_from(TimeSlot).filter(
                    ...
                    Appointment.time_slot_id == None, TimeSlot.date_time > datetime.datetime.now()).offset(offset).limit(limit).order_by(TimeSlot.date_time))).unique().all()), key=lambda t: t.date_time)
        except sqlalchemy.exc.SQLAlchemyError as e:
            await session.rollback()
            return None
